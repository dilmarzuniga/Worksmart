<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Frameset//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-frameset.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
<title>Work Smart Administrador</title>
</head>

<frameset rows="*" cols="235,*" framespacing="0" frameborder="no" border="0" bordercolor="#103649">
  <frame src="menu.php" name="leftFrame" scrolling="No" noresize="noresize" id="leftFrame" title="leftFrame" />
  <frame src="main.html" name="mainFrame" id="mainFrame" title="mainFrame" />
</frameset>
<?php
/*session_start();
@user=$_SESSION['usuario_valido'];
@accion=$_GET['accion'];
if($accion=='close'){
echo("Inicie sesion para continuar<a href='login.php'>clic aqui</a> para volver");
}
if($accion=='ver'){
if(strlen($user)==0){
echo("<script>alert('Inicie sesion para continuar');</script>");
}
}*/
?>
<noframes><body>
</body>
</noframes></html>
